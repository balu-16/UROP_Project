...
# Arctic Embed models are instruction-tuned: queries must carry this prefix,
# documents are embedded without it (per the model card).
...
class EmbeddingService:
    ...
    async def embed_query(self, text: str) -> np.ndarray:
        if self.settings.embedding_model_name.startswith(
            "Snowflake/snowflake-arctic-embed"
        ) and not text.startswith(ARCTIC_QUERY_PREFIX):
